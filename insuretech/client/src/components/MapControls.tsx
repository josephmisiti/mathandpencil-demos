...
interface MapControlsProps {
  ...
  roofAnalysisEnabled: boolean;
  onRoofAnalysisToggle: (enabled: boolean) => void;
      ...
      <div className="bg-white rounded-lg shadow-lg border border-gray-200 min-w-64">
        ...
        {isOpen && (
          <div className="border-t border-gray-200">
            ...
            <div className="px-4 py-3 border-t border-gray-200">
              <div className="flex items-center justify-between">
                <label htmlFor="roof-analysis-toggle" className="text-sm font-medium text-gray-700">
                  Roof Analysis
                </label>
                <button
                  id="roof-analysis-toggle"
                  type="button"
                  onClick={() => onRoofAnalysisToggle(!roofAnalysisEnabled)}
                  aria-pressed={roofAnalysisEnabled}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
                    roofAnalysisEnabled ? "bg-blue-600" : "bg-gray-300"
                  }`}
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-200 ${
                      roofAnalysisEnabled ? "translate-x-6" : "translate-x-1"
                    }`}
                  />
                </button>
              </div>
