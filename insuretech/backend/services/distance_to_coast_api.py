...
def find_nearest_coastline(lat: float, lng: float, radius_miles: float = 5.0):
    ...
    try:
        ...
        # First, find the absolute nearest point
        nearest_query = """
            SELECT
                id,
                lat,
        ...
        cursor.execute(nearest_query, (lat, lng, lat))
        ...
        nearest_id, nearest_lat, nearest_lng, nearest_distance = nearest_result

        # Now get all points within the radius of the query point
        radius_query = """
            SELECT
                id,
                lat,
        ...
        # Convert radius results to list of dicts (only lat/lng)
        coastline_points = []
        if results:
            for row in results:
