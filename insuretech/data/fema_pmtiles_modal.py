...
STORAGE_ROOT = "/cache"
TARGET_FIPS = "02"  # Alaska for testing
...
def convert_gdb_to_fgb(fips: str, file_name: str):
    ...
    raw_fgb_path = os.path.join(output_dir, f"{state_name}_raw.fgb")
    
    if os.path.exists(fgb_path) and os.path.exists(raw_fgb_path):
        logger.info(f"SKIP: {fgb_path} already exists")
        return {"fips": fips, "status": "skipped", "fgb_path": fgb_path, "raw_fgb_path": raw_fgb_path}
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            ...
            # Filter to create cleaned version
...
def create_pmtiles(fips: str, fgb_path: str, raw_fgb_path: str):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            ...
            run_command(f"cp '{raw_fgb_path}' '{temp_raw_fgb}'")
            
            # Create z0-10 tiles (filtered data)
            temp_z0_10 = os.path.join(temp_dir, f"{state_name}_z0_10.pmtiles")
            cmd = (
            ...
            # Create z10-16 tiles (filtered data)
            temp_z10_16 = os.path.join(temp_dir, f"{state_name}_z10_16.pmtiles")
            cmd = (
...
def main():
    ...
    alaska_data = manifest[TARGET_FIPS]
    file_name = alaska_data["file_name"]
    
    logger.info(f"Processing Alaska (FIPS {TARGET_FIPS}): {file_name}")
    ...
    pmtiles_result = create_pmtiles.remote(
        ...
        conversion_result["raw_fgb_path"]
    )
    
    if pmtiles_result["status"] == "success":
        logger.info("--- Pipeline Complete ---")
        logger.info(f"PMTiles created successfully for Alaska (FIPS {TARGET_FIPS})")
        if "tiles" in pmtiles_result:
            for tile_path in pmtiles_result["tiles"]:
                logger.info(f"Created: {tile_path}")
    else:
